...
if __name__ == "__main__":
    ...
    df = dataset[['id', 'geom','timestamp','city', 'angle', 'speed', 'st_astext']]
    ...
    for i in range(len(df.speed)):
        ...
        if df.city[i]==Localization and int(re.findall(r'[+]?(?:\d*\.\d+|\d+)', df.timestamp[i])[1])==Month:
            ...
            e = df.angle[i]; f = df.speed[i]; g = df.st_astext[i]
            res.append(str(a)+';'+str(b)+';'+str(c)+';'+str(d)+';'+str(e)+';'+str(f)+';'+str(g))
    ...
    pathOut = 'Output/'
    nameFile=pathOut+'wind_%s.csv'%Localization
    
    dicList = []
    energy = 0
    keys =  ['id', 'geom','timestamp','city', 'angle', 'speed', 'st_astext']
    for i in range(len(dataset)):
        Time = dataset[i].split(';')[0]
        City = dataset[i].split(';')[3]
        Speed = dataset[i].split(';')[5]
        Angle = dataset[i].split(';')[4]
        Id = dataset[i].split(';')[1]
        St_astext = dataset[i].split(';')[6]
        ...
        dicts = {}
        keys = ['id', 'geom','timestamp','city', 'angle', 'speed', 'st_astext']
        values = [Id, Geom, Time, City, Angle, Speed, St_astext]
