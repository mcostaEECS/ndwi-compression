...
def test_psnr(data, mask, N, W, TH):
    ...
    return res, cr, nb, blocks



def SaveLogs(campaign, res):
    
    #campaign, Test_type, pwr, cpu
    
    pathOut = '/home/marcello-costa/workspace/'
    nameFile=pathOut+'Campaign_%s.csv' %campaign
    
    dicList = []
    energy = 0
    keys = ['test_type', 'psnr', 'cr', 'nb']
...
if __name__ == "__main__":
    ...
    data1=sio.loadmat(path+'nir-v'+'.mat')  
    data1=data1['NIR']
    
    mask1=sio.loadmat(path+'mask-v'+'.mat')  
    mask1=mask1['MASK']
    
    data2=sio.loadmat(path+'nir-w'+'.mat')  
    data2=data2['NIR']
    
    mask2=sio.loadmat(path+'mask-w'+'.mat')  
    mask2=mask2['MASK']
    
    data3=sio.loadmat(path+'nir-sea'+'.mat')  
    data3=data3['NIR']
    
    mask3=sio.loadmat(path+'mask-sea'+'.mat')  
    mask3=mask3['MASK']
    ...
    pfa_min = 0.5; pfa_max = 1 # AR1
    pfa_range = np.arange(pfa_min, pfa_max,0.05)#[::-1]
    ...
    for k in range(len(season)):
        ...
        for i in pfa_range:
            th = round(i,2)
            [Psnr, Cr, Nb,b]=test_psnr(data[k], mask[k], N, W, th)
            PSNR.append(Psnr); CR.append(Cr); NB.append(Nb);Test.append(season[k])
            RES=(PSNR,CR,NB,Test)
