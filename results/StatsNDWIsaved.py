...
if __name__ == "__main__":
    ...
    if season == 'summer':
        ...
        #window = rasterio.windows.Window(500,1500,3000,1000)
    ...
    with rasterio.open('data/%s/%s.tif'%(season,data[1])) as src:
        band_green = src.read(1, window=window)
        
    with rasterio.open('data/%s/%s.tif'%(season,data[1])) as src:
        band_red = src.read(1, window=window)
        
    with rasterio.open('data/%s/%s.tif'%(season,data[3])) as src:
        band_nir = src.read(1, window=window)
        
    with rasterio.open('data/%s/%s.tif'%(season,data[4])) as src:
        band_ndwi = src.read(1, window=window)

    
   
    
    print('mean_green:', band_green.mean())
    print('mean_nir:', band_nir.mean())
    print('mean_ndwi:', band_ndwi.mean())
    
    
    print(band_ndwi.shape)    
    
    f, ax1 = plt.subplots()
    ax1.imshow(band_ndwi)
    plt.axis("off") 

    
    
    path='/home/marcello-costa/workspace/'
    plt.savefig(path + 'Class_ndwi_%s.png'%(season), dpi=350,bbox_inches='tight',transparent=True, pad_inches=0)
    plt.tight_layout()
    ...
    # # Blue
    # show_hist(band_ndwi, bins=200, histtype='step',
