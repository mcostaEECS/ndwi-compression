...
if __name__ == "__main__":
    ...
    #path = 'data/summer/'; window = rasterio.windows.Window(1000,1000,3000,1000)
    #path = 'data/winter/'; window = rasterio.windows.Window(800,1400,3000,1000)
    #path = 'data/sea/'; window = rasterio.windows.Window(500,1500,3000,1000) # sea check in the winter (Västerskaten)
    path = 'data/sea/'; window = rasterio.windows.Window(1000,1400,3000,1000) # sea check in the winter (Västerskaten)
...
#     # Set spatial characteristics of the output object to mirror the input
    ...
    with rasterio.open(path+'blue.tif', 'w',**kwargs) as dst:
        dst.write_band(1, band_blue.astype(rasterio.float32),window=window)
        
    with rasterio.open(path+'green.tif', 'w',**kwargs) as dst:
        dst.write_band(1, band_green.astype(rasterio.float32),window=window)
    
    with rasterio.open(path+'red.tif', 'w',**kwargs) as dst:
        dst.write_band(1, band_red.astype(rasterio.float32),window=window)
    
    with rasterio.open(path+'nir.tif', 'w',**kwargs) as dst:
        dst.write_band(1, band_nir.astype(rasterio.float32),window=window)
       
    with rasterio.open(path+'ndwi.tif', 'w',**kwargs) as dst:
        dst.write_band(1, ndwi.astype(rasterio.float32),window=window)
