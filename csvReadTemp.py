...
if __name__ == "__main__":
    ...
    df = dataset[['timestamp','city', 'temperature']]
    ...
    for i in range(len(df.temperature)):
        ...
        if df.city[i]=='Stockholm' and int(re.findall(r'[+]?(?:\d*\.\d+|\d+)', df.timestamp[i])[1])==6:
            a = df.timestamp[i]
            b = df.city[i]
            c = df.temperature[i]
            dat.append(str(a)+';'+str(b)+';'+str(c))
            res.append(str(b)+';'+str(c))
            
  
    fmt = "%Y-%m-%d %H:%M:%S"
    
    dataset1 = sorted(dat, key=lambda s: datetime.strptime(s, fmt+';'+s[20:50]))
