...
def test(a,b, temp, wind, N, K):
    ...
    return Arraylist[0], idxNcomp, idxComp
...
if __name__ == "__main__":
     ...
     CD=[]; lmp=[]; ump=[]; count = []; C=[]; U=[]
     i = 0
     while i < len(Itest):
        
         if i <= len(Itest)-2:
             [OUT0,c,u]=test(Itest[i+1], Itest[i], temp, wind, N, K)
             CD.append(OUT0); C.append(c); U.append(u)
             #lmp.append(lmp_time); ump.append(ump_time); count.append(cnt)
        
         else:
             [OUT0,c,u]=test(Itest[len(Itest)-1], Itest[len(Itest)-2], temp, wind, N, K)
             CD.append(OUT0); C.append(c); U.append(u)
             #lmp.append(lmp_time); ump.append(ump_time); count.append(cnt)
         i = i + 1
     ...
     path = 'COMPRESS/'
     name_file = path+'nir'+'.mat'  
     scipy.io.savemat(name_file, {'NIR':band_nir})
     
     name_file = path+'mask'+'.mat'  
     scipy.io.savemat(name_file, {'MASK':ICD})
    ...
    #      ax = fig.gca()
    #      major_ticks = np.arange(0, 501,10)
    ...
    # Major ticks every 20, minor ticks every 5
     ...
     #minor_ticks = np.arange(0, 501, 10)
