...
def get_data(file, temp, wind, Localization): 
    #path = 'data_sum/'
    path = 'data_sea/'
    ...
    for i in range(len(df.temperature)):
        ...
        if d==date_rounded and df.city[i]==Localization:
...
if __name__ == "__main__":
    ...
    for i in range(len(data)):
        [date, file,  avg, temp, wind]=get_data(data[i],path+exoge[1],path+exoge[0], Localization)
